...
class OptrisDataset(DictDataset):
    def __init__(self, filepath, delimiter = ','):  # auto builds dataaset
        super().__init__()
            ...
            for num, row in enumerate(reader):
                try:  # extract values for time + each ROI
                    ...
                    for num, series in enumerate(series_headers):
                        if delimiter == ';':
                            # Optris version 3.24 now uses ';' as delimiter
                            working_column = num + 1
                            value = float(row[working_column].replace(',', '.'))
                        else:
